...
(defmethod mut/mutate 'login/attempt-login [{:keys [state]} k {:keys [uid]}]
  {:remote true
   :action (fn [] (swap! state assoc
                    :current-user {:id uid :name "???"}
                    :server-down false))})

(defmethod mut/mutate 'login/server-down [{:keys [state]} k p]
  {:action (fn [] (swap! state assoc :server-down true))})

(defmethod mut/mutate 'login/login-complete [{:keys [state]} k p]
  {:action (fn []
...
(defmethod mut/mutate 'login/logout [{:keys [state]} k p]
  {:remote true
   :action (fn []
             (swap! state assoc
               :current-user {}
